...
def main():
    ...
    if result != CUDA_SUCCESS:
        ...
        logging.debug("cuInit failed with error code %d: %s" %
                      (result, error_str.value.decode()))
        raise
    result = cuda.cuDeviceGetCount(ctypes.byref(nGpus))
    if result != CUDA_SUCCESS:
        cuda.cuGetErrorString(result, ctypes.byref(error_str))
        logging.debug("cuDeviceGetCount failed with error code %d: %s" %
                      (result, error_str.value.decode()))
        raise
    ...
    for i in range(nGpus.value):
        ...
        if result != CUDA_SUCCESS:
            cuda.cuGetErrorString(result, ctypes.byref(error_str))
            logging.debug("cuDeviceGet failed with error code %d: %s" %
                          (result, error_str.value.decode()))
            raise
        ...
        if cuda.cuDeviceComputeCapability(ctypes.byref(cc_major), ctypes.byref(cc_minor), device) == CUDA_SUCCESS:
            logging.debug("  Compute Capability: %d.%d" %
        ...
        if cuda.cuDeviceGetAttribute(ctypes.byref(cores), CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT,
            ...
            logging.debug(
                "  CUDA Cores: %s" % (cores.value * ConvertSMVer2Cores(cc_major.value, cc_minor.value) or "unknown"))
            gpu['cores'] = cores.value * \
                ConvertSMVer2Cores(cc_major.value, cc_minor.value) or "unknown"
            if cuda.cuDeviceGetAttribute(ctypes.byref(threads_per_core),
                ...
                logging.debug("  Concurrent threads: %d" %
        ...
        if cuda.cuDeviceGetAttribute(ctypes.byref(clockrate), CU_DEVICE_ATTRIBUTE_CLOCK_RATE, device) == CUDA_SUCCESS:
            logging.debug("  GPU clock: %g MHz" % (clockrate.value / 1000.))
            gpu['clock'] = clockrate.value / 1000.
        if cuda.cuDeviceGetAttribute(ctypes.byref(clockrate), CU_DEVICE_ATTRIBUTE_MEMORY_CLOCK_RATE,
                                     device) == CUDA_SUCCESS:
            logging.debug("  Memory clock: %g MHz" % (clockrate.value / 1000.))
        try:
        ...
        if result != CUDA_SUCCESS:
            cuda.cuGetErrorString(result, ctypes.byref(error_str))
            logging.debug("cuCtxCreate failed with error code %d: %s" %
                          (result, error_str.value.decode()))
        else:
            try:
                ...
                result = cuda.cuMemGetInfo(
                    ctypes.byref(freeMem), ctypes.byref(totalMem))
            if result == CUDA_SUCCESS:
                logging.debug("  Total Memory: %ld MiB" %
                              (totalMem.value / 1024 ** 2))
                gpu['memory'] = totalMem.value / 1024 ** 2
                logging.debug("  Free Memory: %ld MiB" %
                              (freeMem.value / 1024 ** 2))
                gpu['free'] = freeMem.value / 1024 ** 2
            else:
                cuda.cuGetErrorString(result, ctypes.byref(error_str))
                logging.debug("cuMemGetInfo failed with error code %d: %s" % (
                    result, error_str.value.decode()))
